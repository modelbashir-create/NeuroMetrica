...
struct ContentView: View {
    ...
    // Namespaces for future morphing transitions (iOS 26+)
    ...
    private var toolbarContent: some ToolbarContent {
        ...
        // Right side: settings / inspector / export
    ...
    // MARK: Viewer tools (center group)
    ...
    // MARK: Right-side toolbar buttons
...
// MARK: - iOS 26-style transition helpers (no-op on current OS)
...
struct ViewportView: View {
    ...
    private func viewportOverlays(size: CGSize) -> some View {
        #if swift(>=6.0)
        if #available(iOS 26, macOS 26, *) {
            // Placeholder for future GlassEffectContainer; fallback below
    ...
    private var cornerOverlays: some View {
        ZStack {
            ...
            // Bottom-left: WW/L and slice index
            VStack(alignment: .leading, spacing: 2) {
                Text("W 350  L 40")
...
// MARK: - Crosshair & measurement overlays
...
#Preview("Neurometrica Shell") {
    ContentView(inspectorPresented: .constant(true))
        .environment(ViewerState())
}

#Preview("Viewport Only") {
    ZStack {
        Color.black
        ViewportView(index: 0)
            .padding()
